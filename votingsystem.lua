if not musicPack then
    smlua_audio_utils_replace_sequence(0x3F, 42, 96, "Menu") -- remember to make this compatible with music packs later [not finished]
end
...
local function on_hud_render()
    if gGlobalSyncTable.map_deciding then
        ...
        if gGlobalSyncTable.newtimer > 45 then
            ...
            djui_hud_set_adjusted_color(92, 129, 184, 255)
            ...
            if gGlobalSyncTable.newtimer == 128 then
                djui_chat_message_create("Use \\#f0d946\\C-Left [<] \\#ffffff\\and \\#f0d946\\C-Right [>] \\#ffffff\\to select, and use \\#48cf7a\\B \\#ffffff\\to finalize your vote.")
            end
            ...
            djui_hud_print_text(truncateString(tostring(gLevels[gGlobalSyncTable.course1].name), 9) .. ": " .. tostring(gGlobalSyncTable.course1_v), ((djui_hud_get_screen_width() / 2) - 384) - (djui_hud_measure_text(truncateString(tostring(gLevels[gGlobalSyncTable.course1].name), 9) .. ": " .. tostring(gGlobalSyncTable.course1_v)) / 2), 655, 1)

            djui_hud_print_text(truncateString(tostring(gLevels[gGlobalSyncTable.course2].name), 9) .. ": " .. tostring(gGlobalSyncTable.course2_v), (djui_hud_get_screen_width() / 2) - (djui_hud_measure_text(truncateString(tostring(gLevels[gGlobalSyncTable.course2].name), 9) .. ": " .. tostring(gGlobalSyncTable.course2_v)) / 2), 655, 1)

            djui_hud_print_text(truncateString(tostring(gLevels[gGlobalSyncTable.course3].name), 9) .. ": " .. tostring(gGlobalSyncTable.course3_v), ((djui_hud_get_screen_width() / 2) + 384) - (djui_hud_measure_text(truncateString(tostring(gLevels[gGlobalSyncTable.course3].name), 9) .. ": " .. tostring(gGlobalSyncTable.course3_v)) / 2), 655, 1)
...
local function mario_update(m)
    ...
    if gGlobalSyncTable.map_deciding and gGlobalSyncTable.newtimer > 127 then
        ...
        if not pickedMap then
            if m.controller.buttonPressed & L_CBUTTONS ~= 0 and position > 0 then
                position = position - 1
                play_sound_with_freq_scale(SOUND_MENU_MESSAGE_NEXT_PAGE, gGlobalSoundSource, 1)
            end

            if m.controller.buttonPressed & R_CBUTTONS ~= 0 and position < 2 then
                position = position + 1
                play_sound_with_freq_scale(SOUND_MENU_MESSAGE_NEXT_PAGE, gGlobalSoundSource, 1)
            end
            ...
            if(m.controller.buttonPressed & B_BUTTON) ~= 0 then
                play_sound_with_freq_scale(SOUND_MENU_CAMERA_BUZZ, gGlobalSoundSource, 1) -- Bro

                djui_chat_message_create("You have already voted.")
            end
        end
    end
end
...
local function cancel_everything(sound)
    if gGlobalSyncTable.map_deciding and gGlobalSyncTable.newtimer > 45
