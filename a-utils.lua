...
-- credits to creator of Flood EX for idea and this little snipet of code, thx Murioz

local function slides(m)
    if gNetworkPlayers[0].currLevelNum == LEVEL_PSS
    or gNetworkPlayers[0].currLevelNum == LEVEL_CCM and gNetworkPlayers[0].currAreaIndex == 2 then
        if m.area.terrainType == TERRAIN_SLIDE then
            m.area.terrainType = TERRAIN_STONE
        end

        gLakituState.mode = CAMERA_MODE_8_DIRECTIONS
        gLakituState.defMode = CAMERA_MODE_8_DIRECTIONS
        m.area.camera.mode = CAMERA_MODE_8_DIRECTIONS
    end
end
...
local levelToCourse = {
    ...
    [LEVEL_SA] = COURSE_SA,
    [LEVEL_ENDING] = COURSE_CAKE_END,
    [LEVEL_MC] = COURSE_NONE,
    [LEVEL_CASINO] = COURSE_NONE,
    [LEVEL_CS] = COURSE_NONE
}
...
-- (gMarioStates[i].flags & MARIO_VANISH_CAP) == 0 and
